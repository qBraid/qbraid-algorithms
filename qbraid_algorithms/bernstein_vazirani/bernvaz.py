...
def load_program(bitstring: str | list[int]) -> PyQASMModule:
    """
...
def generate_subroutine(bitstring: str | list[int]) -> None:
    ...
    bernvaz_dst = os.path.join(os.getcwd(), f"bernvaz.qasm")
    shutil.copy(bernvaz_src, bernvaz_dst)
...
def generate_oracle(bitstring: list[int], as_str=False) -> None:
    """
    ...
    oracle_dst = os.path.join(os.getcwd(), f"oracle.qasm")
    ...
    print(f"Oracle 'oracle' has been added to {os.path.join(os.getcwd(), 'oracle.qasm')}")
...
def _generate_replacements(bitstring: str | list[int]) -> dict[str, str]:
    ...
    return {
        "BERNVAZ_SIZE": str(input_size),
        "SECRET_BITSTRING": str(decimal_value)
    }
